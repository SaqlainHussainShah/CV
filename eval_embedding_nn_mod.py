...
def eval_once(saver, placeholders, feat_img, feat_sent):
    with tf.Session() as sess:
        ...
        test_file = 'E:/saqlain/face_test.csv'
        test_file_voice = 'E:/saqlain/wav_test.csv'

        img_test, test_label = read_file_org_test(test_file)
        img_test_voice, _ = read_file_org_test(test_file_voice)
        
        img_test = img_test[:]
        img_test_voice= img_test_voice[:]
        test_label= test_label[:]
        # img_test = img_test[1:28193]
        # test_label = test_label[1:28193]
        # img_test_voice = img_test_voice[1:28193]
        ...
        print("SHape img feat {}".format(feat_img.shape))
        
        print("SHape sent feature {}".format(feat_img.shape))
...
def read_file_org_test(file_name):
    ...
    with open(file_name) as fr:
        ...
        for row in reader:
            s_feat = [float(i) for i in row[:-1]]
            feat_lst.append(s_feat)
            label_lst.append(int(float(row[-1])))
...
def main(_):
    ...
    face, voice, test_labels = eval_once(saver, placeholders, feat1, feat2)
    voice_features = []
    print("started zipping")
    for v,l in zip(voice, test_labels):
        voice_features.append(np.append(v,int(l)))
        
    pd.DataFrame(voice_features).to_csv('wav_features_test_mod.csv', header=None, index=None)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore_path', type=str, default='./orgplot_3/',
    ...
    parser.add_argument('--batch_size', type=int, default=8251,
                        help='Batch size for evaluation.')  # awa 37322 , flo 8189 , sun 14340 # cub 11788 # 15339 apy
    parser.add_argument('--sample_size', type=int, default=5, help='Number of positive pair to sample.')

    FLAGS, unparsed = parser.parse_known_args()
    # face, voice = main(FLAGS)
